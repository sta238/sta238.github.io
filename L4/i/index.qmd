...
- The **relative efficiency** of $\widehat\theta_2$ *with respect to $T_1$* is $\frac{\text{Var}(\widehat\theta_1)}{\text{Var}(\widehat\theta_2)}$
...
::: {#exm-max}
Given $X_1, ...,X_n \overset{iid}{\sim} \cal{U}[0,\theta]$. Which of these unbiased^[See: Estimators, Part 1, Example 5 for $\widehat\theta_1$. Showing that $\widehat\theta_2$ is unbiased is left as a exercise.] estimators of $\theta$ is preferred? 
...
::: {#prp-MSE-bias_var}
If $\widehat\theta$ is any estimator $\widehat\theta$ for $\theta$, then $MSE(\widehat\theta)=\text{Var}(\widehat\theta)+\text{Bias}^2$.
:::
